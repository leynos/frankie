...
use mock_git_ops::MockBddGitOps;
...
fn when_loaded_with_custom_limit(state: &CommitHistoryLimitWorld, limit: usize) -> StepResult {
    ...
    // Clamp limit to minimum of 1 to match production behavior
    let effective_limit = limit.max(1);

    if effective_limit != expected_limit {
        return Err("effective limit should match expected_limit from Given step");
    }

    let mock_commit_count = if effective_limit == 1 {
        1
    } else {
        state
            .mock_commit_count
    ...
    let result = load_state(&git_ops, &commit_sha, &file_path, effective_limit)?;
    state.state.set(result);
    Ok(())
}
...
#[then("the loaded history contains {expected} commit")]
fn then_history_count_singular(state: &CommitHistoryLimitWorld, expected: usize) -> StepResult {
    then_history_count(state, expected)
}
