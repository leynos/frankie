...
// Import truncation step functions from submodule (used by scenario macros)
#[expect(
    unused_imports,
    reason = "step functions are invoked by scenario macros, not directly called"
)]
use comment_detail_bdd_support::{
    given_comment_producing_many_lines, given_comment_with_blank_lines, then_blank_lines_preserved,
    then_last_line_is_ellipsis, then_output_has_max_lines, when_view_rendered_with_max_height,
};
...
// Note: The step functions are defined in comment_detail_bdd_support::truncation_steps
