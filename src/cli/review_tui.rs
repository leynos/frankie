...
use frankie::tui::{
    ReplyDraftConfig, ReviewApp, TimeTravelContext, set_git_ops_context, set_initial_reviews,
...
pub async fn run(config: &FrankieConfig) -> Result<(), IntakeError> {
    ...
    let reply_draft_config =
        ReplyDraftConfig::new(config.reply_max_length, config.reply_templates.clone());
