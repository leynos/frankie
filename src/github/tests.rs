...
fn page_info_accessors() {
    let info = PageInfo::new(3, 25)
        .with_total_pages(Some(10))
        .with_has_next(true)
        .with_has_prev(true);
    ...
    gateway
        ...
        .returning(|_, _| {
            Ok(PaginatedPullRequests {
                ...
                page_info: PageInfo::new(1, 50).with_total_pages(Some(1)),
                rate_limit: None,
            })
        });
...
async fn lists_pull_requests_with_rate_limit_info() {
    ...
    gateway
        ...
        .returning(|_, _| {
            Ok(PaginatedPullRequests {
                items: vec![],
                page_info: PageInfo::new(1, 50).with_total_pages(Some(0)),
