...
fn fixture_comments() -> Vec<ReviewComment> {
    (1..=18)
        .map(|id| {
            ...
            let file_path = format!("src/component_{:02}.rs", (id % 5) + 1);
...
fn auto_exit_duration_ms() -> Option<u64> {
    parse_auto_exit_duration_ms_from_args()
        .or_else(|| env::var("TUI_RESIZE_FIXTURE_AUTO_EXIT_MS")
            .ok()
...
async fn main() {
    ...
    let program = match program {
        Ok(program) => program,
        Err(error) => {
            eprintln!("failed to build TUI program: {error}");
            process::exit(1);
        }
    };
    ...
    let result = match auto_exit_duration_ms() {
        Some(duration_ms) => {
            tokio::select! {
                run_result = run_future => run_result,
                _ = tokio::time::sleep(Duration::from_millis(duration_ms)) => Ok(ReviewApp::empty()),
            }
        }
    ...
    if let Err(error) = result {
        eprintln!("failed to run TUI program: {error}");
        process::exit(1);
    }
}
